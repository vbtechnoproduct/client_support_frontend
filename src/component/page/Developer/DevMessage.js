...
import { getMessageDataDev, getMessageOldChatDev, closeTicketDev, imgUploadDevChat } from '../../redux/devSlice/devMessageSlice'
...
export default function DevMessage() {
    const replayBorderColor = ["#007bfc", "#007bfc", "#53bdeb", "#a5b337", "#53bdeb", "#06cf9c"]
    const randomIndex = Math.floor(Math.random() * replayBorderColor?.length);
    const { dialogue, dialogueType } = useSelector((state) => state.dialogue);
    const { messageDev, totalMessageDev, messageDevOldChat, totalMessageDevOldChat } = useSelector((state) => state.devMessage);
    const bgProfileColor = ["rgb(255, 58, 110)", "rgb(255, 58, 110)", "rgb(108, 117, 125", "rgb(255, 162, 29)", "rgb(111, 217, 67)"]
    ...
    const getDeveloperData = JSON.parse(sessionStorage.getItem("devDetails"));
    const safeObjectValues = (obj) => obj && Object.values(obj);

    useEffect(() => {
        if (messageDev?.length > 0) {
            setData(messageDev)
            setReceiverDetails(messageDev[0]?.receiver)
            setSenderDetails(messageDev[0]?.sender)
            setUserDetailShow(messageDev[0])
            setAdminData(messageDev[0]?.admin)
            setTicketStatus(messageDev[0]?.ticket?.status)
    ...
    useEffect(() => {
        if (Object?.values(userDetailShow)?.length > 0) {
            const socket = io.connect(baseURL, {
                ...
                query: { globalRoom: userDetailShow && userDetailShow?.ticket?.ticketId },
            });
            socketRef.current = socket ? socket : null;
            socketRef.current.on("connect", () => {
                console.log("Socket connected");
                socketRef.current.emit("ticket", userDetailShow && userDetailShow?.ticket?.ticketId);
            });
    ...
    useEffect(() => {
        socketRef.current &&
            socketRef.current.on("message", (messageShow) => {
                playRingtone()
                if (messageShow?.chat?.messageType === 1 && messageShow?.chat?.chatTopic === userDetailShow?._id) {
                ...
                if (messageShow?.chatTopic === userDetailShow?._id) {
                    if ((messageShow?.messageType === 2 || messageShow?.messageType === 3) && messageShow?.role !== "developer") {
                        setAllChatData((prevState) => [...prevState, messageShow]);
                    }
                    if (messageShow?.messageType === 2 || messageShow?.messageType === 3) {
    ...
    useEffect(() => {
        const payload = {
            ...
            developerId: getDeveloperData?._id
        }
    ...
    const formetAgoDate = (createdAtTimestamp) => {
        if (createdAtTimestamp) {
            ...
            const diffInMinutes = now.diff(dayjs(createdAtTimestamp).tz("Asia/Kolkata"), "minute");

            if (diffInMinutes < 1) {
                return "just now";
            } else if (diffInMinutes < 60) {
                return diffInMinutes + " min ago";
            } else if (now.diff(dayjs(createdAtTimestamp).tz("Asia/Kolkata"), "hour") >= 24) {
                return dayjs(createdAtTimestamp).tz("Asia/Kolkata").format("DD MMM, YYYY");
            } else {
                return now.diff(dayjs(createdAtTimestamp).tz("Asia/Kolkata"), "hour") + " hour ago";
            }
        }
    };
    ...
    const handleSendMessage = () => {
        ...
        const mentionDisplayNames = mentionsData?.map(mention => mention?.display);
    ...
    const handleUpload = (e) => {
        if (e.target.files[0].type === "application/x-zip-compressed") {
            ...
            formData.append("receiverId", receiverDetails?._id && receiverDetails?._id);
            formData.append("receiverRole", "client");
            formData.append("senderRole", "developer");
            formData.append("messageType", 3);
            formData.append("image", e.target.files[0]);
            const payload = {
                data: formData,
            }
            dispatch(imgUploadDevChat(payload))
    ...
    const fetch = () => {
        setTimeout(() => {
            if (hasMore) {
                ...
                setStart(start);
                setLimit(limit + 100);
                if (limit + 100 >= totalMessageDevOldChat) {
                    setHasMore(false);
                }
            }
        }, 800);
    ...
    const fetchUser = () => {
        setTimeout(() => {
            if (hasMoreUser) {
                ...
                if (userProfileStart + 20 >= totalMessageDev) {
    ...
    useEffect(() => {
        const uniqueObject = {};
        const uniqueData = allChatData && allChatData?.filter((item) => {
    ...
    useEffect(() => {
        const lastChat = messageChat ? messageChat : allChatData[allChatData?.length - 1];
        const messageToUpdate = data?.find(message => message?._id === lastChat.chatTopic);
        if (messageToUpdate) {
            const updateData = {
                ...messageToUpdate,
                chat: {
            ...
            const filteredAllData = data?.filter(data => data?._id !== updateData?._id);
    ...
    const handleSearch = (event, filter) => {
        ...
        let searchValue = search ? search.toLowerCase() : event?.target?.value?.toLowerCase();
        if (searchValue || filter) {
            const filteredData = data.filter((item) => {
                if (filter) {
                    ...
                    const receiverName = item.receiver ? item.receiver.name.toLowerCase() : '';
    ...
    const hadnleTicketClose = async () => {
        ...
        let response = await dispatch(closeTicketDev(payload)).unwrap();
        if (response?.status) {
            setTicketStatus(2)
        }
    }
            ...
            <div className='messagePage adminPageChat'>
                <div className='messageConetnt'>
                    <div className='row align-items-start moMessageBody'>
                        <div className={`leftSideChat col-md-3 col-12 md-p-0 `} style={{ display: `${moMessageShowBody === true ? "none" : "block"}` }}>
                            <div className='chatUserShow'>
                                <div className='topHeadShowChat'>
                                    <div className='backButtonClick' onClick={() => hadnleShowMModel()}>
                                        <IoChevronBackSharp />
                                    </div>
                                    <h6>Messages</h6>
                                        ...
                                        >
                                            {
                                                data&&data?.slice()?.reverse()?.map((item, index) => {
                                                    return (
                                                        <div className={`userProfileContent ${item?._id === (safeObjectValues(userDetailShow)?.length > 0 && userDetailShow?._id) ? "activeChat" : ""}`} onClick={() => handleOnChat(item, AvtarImg[index])} >
                                                            <div className='userProfileBox grupeProfile'>
                                                                <div className='profileImgGrupe'>
                                                                    ...
                                                                    <span className='nameImgProfile' style={{ backgroundColor: `${item?.receiver?.colorCode}` }}>{item?.receiver?.name ? item?.receiver?.name?.charAt(0) : ""}</span>
                                                                    <div className={`moOnline `}><div className={`${item?.receiver?.isOnline ? "onlineButton" : "offlineButton"}`}></div></div>
                                                                ...
                                                                <div className='profileText'>
                                                                    ...
                                                                    <div className='timeTextShow'>
                                                                        <h4>{formetAgoDate(item?.chat?.date ? item?.chat?.date : "")}</h4>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                        ...
                        <div className={`chatMessageShow  col-md-6 col-12 md-p-0 d-flex align-item-center justify-content-center  `}  >
                            <div className='chatDataShow' style={{ display: `${moMessageShowBody === true ? "block" : ""}` }}>
                                {
                                    chatData?.length > 0 ?
                                        (
                                            <>
                                                <div className='topchatHead'>
                                                    {
                                                        moMessageShowBody &&
                                                        <div className='backCharMessage' onClick={() => handleMobileChatView()}>
                                                            <IoChevronBack />
                                                        </div>
                                                    }
                                                    <div className='chatImgShow'>
                                                        <div className='chatBodyGrupeImg'>
                                                            {
                                                                <span className='nameImgProfile' style={{ backgroundColor: `${receiverDetails?.colorCode}` }}>{receiverDetails?.name ? receiverDetails?.name?.charAt(0) : ""}</span>
                                                            }
                                                            <h6> {safeObjectValues(receiverDetails)?.length > 0 && receiverDetails?.name}</h6>
                                                            {/* {
                                            safeObjectValues(senderDetails)?.length > 0 && senderDetails?.image ?
                                                ...
                                                <div className='chatDataBody ' id="messages__listData" >
                                                    <div className='allChatShow' id="scrollableDiv" ref={chatContainerRef}>
                                                        <InfiniteScroll
                                                            ...
                                                            endMessage={
                                                                <p style={{ color: "white", textAlign: "center", backgroundColor: "#32999396", padding: "6px" }}>
                                                                    No more messages
                                                                </p>
                                                            }
                                                            style={{
                                                        ...
                                                        >
                                                            {
                                                                ...
                                                                &&
                                                                chatData?.slice(0)?.reverse()?.map((item, index) => {
                                                                    return (
                                                                        <>
                                                                            {
                                                                                item?.messageType == 4 ?
                                                                                    <h6 className='mx-auto py-3 px-3 text-white  rounded-3' style={{ backgroundColor:"rgb(108, 117, 125)"}}>{item?.message}</h6>:
                                                                                item?.role === "client" || item?.role === "admin" ?
                                                                                    <div className='leftContent' id={item?._id} ref={chatContainerRef}>
                                                                                        {
                                                                                            item?.messageType === 1 ?
                                                                                                <div className='leftChat' ref={chatContainerRef}>
                                                                                                    <div className='chatDataProfile'>
                                                                                                        {
                                                                                                            item?.role === "client" ?
                                                                                                                <span className='nameImgProfile' style={{ backgroundColor: `${receiverDetails?.colorCode}` }}>{receiverDetails?.name ? receiverDetails?.name?.charAt(0) : ""}</span>
                                                                                                                :
                                                                                                                <img src={adminData?.image} className='profileImg' />
                                                                                                        }
                                                                                                    </div>
                                                                                                    <div className='messageShow' >
                                                                                                        <div className={`messageBodyShow ${item?.message.includes('@client') ? 'mentionStyle' : ''} ${item?.isReply === true ? "replayMessageBody" : ""}`} >

                                                                                                            <button className='downarrowMessage ' onClick={() => hadnleReplayModelOpen(item)}><LuReply /></button>
                                                                                                            {/* <p>{item?.message}</p> */}
                                                                                                            {
                                                                                                                item?.isReply === true ?
                                                                                                                    <div className='replayMessageText' onClick={() => handleScrollViewMessage(item)} >
                                                                                                                        <h6 style={{ borderColor: `${replayBorderColor[randomIndex]}` }}>{item?.replyMessageId?.message}</h6>
                                                                                                                        {
                                                                                                                            item?.message.split(new RegExp(`(@${senderDetails?.name}|@${receiverDetails?.name})`)).map((part, index) => (
                                                                                                                                part.startsWith('@') && part !== `@${senderDetails?.name}` && part !== `@${receiverDetails?.name}`
                                                                                                                                    ? <span key={index} className='mentionName'>{part}</span>
                                                                                                        ...
                                                                                                        <div className='UserameShow'>
                                                                                                            <h6>{formatChatDate(item?.createdAt) + " " + `(${item?.role?.charAt(0)?.toUpperCase()})`}</h6>
                                                                                                        </div>
                                                                                                    </div>
                                                                                                </div>
                                                                                                : item?.messageType === 2 ?
                                                                                                    <div className='leftChat messageShowImg' id="messages__listData" ref={chatContainerRef}>
                                                                                                        <div className='messageShow'>
                                                                                                            <div className='messageBodyShow'>
                                                                                                                {/* <NavLink target={'_blank'} to={item?.image} style={{ cursor: "pointer" }}> */}
                                                                                                                <img src={item?.image ? item?.image : NoImg} id="chatImg" onClick={() => dispatch(openDialog({ type: "imageView", data: item?.image }))} />
                                                                                                                {/* </NavLink> */}
                                                                                                            </div>
                                                                                                            <div className='UserameShow'>
                                                                                                                <h6>{formatChatDate(item?.createdAt)}</h6>
                                                                                                            </div>
                                                                                                        </div>
                                                                                                        <div className='chatDataProfile'>
                                                                                                            {
                                                                                                                item?.role === "admin" && adminData?.image ?
                                                                                                                    <img src={adminData?.image} id="img" className='profileImg' />
                                                                                                        ...
                                                                                                        <div className='messageShow'>
                                                                                                            <div className='messageBodyShow'>
                                                                                                                <div className='zipfileShow'>
                                                                                                                    ...
                                                                                                                    <h6>{zipNameShow(item?.image)}</h6>
                                                                                                                    <Button
                                                                                                                        className={"closeModel"}
                                                                                                                        onClick={() => handleZipDownload(item?.image, zipNameShow(item?.image))}
                                                                                                                        bIcon={<MdOutlineFileDownload />} />
                                                                                                                </div>
                                                                                                            </div>
                                                                                                            <div className='UserameShow'>
                                                                                                                <h6>{formatChatDate(item?.createdAt)}</h6>
                                                                                    ...
                                                                                    :
                                                                                    <div className='rightContent' id={item?._id} ref={chatContainerRef}>

                                                                                        {item?.messageType === 1 ?
                                                                                            <div className='rightChat' id="messages__listData" >
                                                                                                <div className='messageShow'>
                                                                                                    <div className={`messageBodyShow ${item?.isReply === true ? "replayMessageBody" : ""}`} id={item?._id} >
                                                                                                        {/* <p>{item?.message}</p> */}
                                                                                                        <button className='downarrowMessage downarrowMessageRight' onClick={() => hadnleReplayModelOpen(item)}><LuReply /></button>
                                                                                                        {/* {item?.message.split(new RegExp(`(@${senderDetails?.name}|@${receiverDetails?.name})`)).map((part, index) => (
                                                                                                        part.startsWith('@') && part !== `@${senderDetails?.name}` && part !== `@${receiverDetails?.name}`
                                                                                                            ? part && <span key={index} className='mentionName'>{part}</span>
                                                                                                            : part === `@${senderDetails?.name}` || part === `@${receiverDetails?.name}`
                                                                                                ...
                                                                                                <div className='chatDataProfile'>
                                                                                                {console.log("getDeveloperData?.image",getDeveloperData?.image)}
                                                                                                    <img src={ getDeveloperData?.image} id="img" className='profileImg' />
                                                                                                </div>
                                                                                            </div>
                                                                                            : item?.messageType === 2 ?
                                                                                                <div className='rightChat messageShowImg' id="messages__listData" ref={chatContainerRef}>
                                                                                                    <div className='messageShow'>
                                                                                                        <div className='messageBodyShow'>
                                                                                                            {/* <NavLink target={'_blank'} to={item?.image} style={{ cursor: "pointer" }}> */}
                                                                                                            <img src={item?.image ? item?.image : NoImg} id="chatImg" onClick={() => dispatch(openDialog({ type: "imageView", data: item?.image }))} />
                                                                                                ...
                                                                                                <div className='rightChat messageShowImg' id="messages__listData" ref={chatContainerRef}>
                                                                                                    <div className='messageShow'>
                                                                                                        <div className='messageBodyShow'>
                                                                                                            <div className='zipfileShow'>
                                                                                                                <BsFileEarmarkZipFill />
                                                                                                                <h6>{item?.name ? item?.name : zipNameShow(item?.image)}</h6>
                                                                                                                <Button
                                                                                                                    className={"closeModel"}
                                                                                                                    onClick={() => handleZipDownload(item?.image, zipNameShow(item?.image))}
                                                                                                                    bIcon={<MdOutlineFileDownload />} />
                                        ...
                                        <div className='notfound' style={{ backgroundColor: "#e3e8e7", height: "100vh" }}>Not Messages...</div>
                                }
                                {
                                    emojiShow && (
                                ...
                                <div className='chatDataFooter'>
                                    {
                                        showReplay && (
                                            <div className='replayMessage'>
                                                <div className='replayMessageShow' style={{ borderColor: `${replayBorderColor[randomIndex]}` }}>
                                                    <h6>{showReplay?.message}</h6>
                                                    <button onClick={() => setShowReplay(null)}><MdOutlineClose /></button>
                                                </div>
                                            </div>
                                        )
                                    }
                                            ...
                                            >
                                                <Mention
                                                    ...
                                                    renderSuggestion={(suggestion, search, highlightedDisplay, index) => (
                                                        <div className={`userMentionShow ${index === focusedIndex ? 'selectedMention' : ''}`} style={{ backgroundColor: `${index == focusedIndex ? "red" : ""}` }} >
                                                            {
                        ...
                        <div className={` col-md-3 col-12 chatUserDetails `} id="responsiveViewDetails">
                            <div className='appInfoBg' style={{ display: "none" }} id="appInfoBgResponsive"></div>
                            <div className='appInfo adminInfo'>
                                ...
                                <div className='appInfoHead'>
                                    <span>App Name</span>
                                    <h5>{userDetailShow?.ticket?.appName ? userDetailShow?.ticket?.appName : ""}</h5>
                                </div>
                                            ...
                                            <div className='statusShow'>
                                                {
                                                    receiverDetails &&
                                                    <>
                                                        <div className={`${receiverDetails?.isOnline ? "onlineButton" : "offlineButton"}`}></div>
                                                        <h5>{receiverDetails ? (receiverDetails?.isOnline ? "Online" : "Offline") : ""}</h5>
                                                    </>
                                                }
                                            </div>
                                            <span className='nameImgProfile' style={{ backgroundColor: `${receiverDetails?.colorCode}` }}>{receiverDetails?.name ? receiverDetails?.name?.charAt(0) : ""}</span>
                                        ...
                                        <div className='statusShow'>
                                            {
                                                senderDetails &&
                                                <>
                                                    <div className={`${senderDetails?.isOnline ? "onlineButton" : "offlineButton"}`}></div>

                                                    <h5>{senderDetails ? (senderDetails?.isOnline ? "Online" : "Offline") : ""}</h5>
                                                </>
                                            }
                                        </div>
                                ...
                                <div className='appInfoFooter'>
                                    <h6>Information:</h6>
                                    <ul>
                                        <li><span>Ticket Id:</span><h5>{userDetailShow?.ticket?.ticketId ? userDetailShow?.ticket?.ticketId : ""}</h5></li>
                                        <li><span>Date</span><h5>{userDetailShow?.createdAt ? dayjs(userDetailShow?.createdAt).format("DD MMM, YYYY") : ""}</h5></li>
                                        <li><span>Issue Description</span><h5>{userDetailShow?.ticket?.issueDescription ? userDetailShow?.ticket?.issueDescription : ""}</h5></li>
                                    </ul>
